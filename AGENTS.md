...
- The **only** LLM path in this app is the Codex Agent SDK (`@anthropic-ai/Codex-agent-sdk`) authenticated with the OAuth **subscription** token (`sk-ant-oat*`) stored under `anthropic_auth_token` in the encrypted credentials store.
