...
export function MarketplaceLeadForm({ compact = false }: { compact?: boolean }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const r = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ ...payload, source: "marketplace-page", turnstileToken }),
      });
      if (!r.ok) throw new Error(await r.text());
      setState("ok");
      setMsg("Thanks — we'll review your marketplace idea and reply within 1 business day.");
  ...
  return (
    <form onSubmit={onSubmit} className={`glass ${compact ? "p-6 md:p-7" : "p-8 md:p-10"} space-y-5`}>
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <Input name="company" label="Company / product name" required />
        <Input name="phone" label="Phone (Singapore)" />
      </div>
      <div>
        <label className="kicker block mb-2" htmlFor="mkt-brief">Tell us about your marketplace</label>
        <textarea
          id="mkt-brief"
          name="message"
          required
          ...
          placeholder="E.g. A two-sided marketplace matching [suppliers] with [buyers]. We want a web app first, then iOS and Android. We have / don't yet have designs and an API…"
        />
      </div>
      <div className="flex items-center justify-between gap-4 flex-wrap pt-1">
        <p className="text-xs text-(--color-muted) font-mono">
          [ Free 30-min scoping call · we reply within 1 business day ]
        </p>
        <button
          type="submit"
        ...
        >
          {state === "sending" ? "Sending…" : "Book my scoping call →"}
        </button>
      </div>
      {msg && (
        <p
