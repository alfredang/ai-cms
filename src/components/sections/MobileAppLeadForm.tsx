...
export function MobileAppLeadForm({ compact = false }: { compact?: boolean }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("ok");
      setMsg("Thanks — we'll review your app idea and reply within 1 business day.");
  ...
  return (
    <form onSubmit={onSubmit} className={`glass ${compact ? "p-6 md:p-7" : "p-8 md:p-10"} space-y-5`}>
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <Input name="company" label="Company / app name" required />
        <Input name="phone" label="Phone (Singapore)" />
      </div>
      <div>
        <label className="kicker block mb-2" htmlFor="app-brief">Tell us about your app</label>
        <textarea
          id="app-brief"
          name="message"
          required
          ...
          placeholder="E.g. A native iOS / Android app that does […]. We need it on the App Store and Google Play. We have / don't yet have designs and a backend…"
        />
      </div>
