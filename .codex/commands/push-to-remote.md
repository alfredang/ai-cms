...
1. **Sanity-check env**. Confirm both `REMOTE_SYNC_URL` and `SYNC_API_TOKEN` are present in the local `.env`. If either is missing, explain the setup steps from [.codex/skills/remote-db-sync/SKILL.md](../skills/remote-db-sync/SKILL.md) and stop.
